...
Static bundle on **Cloudflare Pages** at `app.<domain>`. Backend must be live and
...
| Setting | Value |
|---|---|
| Project name | `swp-contractor` |
| Production branch | `dev` |
| Build command | `bun run build` |
| Output directory | `dist` |

Then **Custom domains → Set up a domain → `app.<domain>`**. Cloudflare creates the CNAME
...
| Kind | Name | Value |
|---|---|---|
| Variable | `VITE_APP_API_URL` | `https://api.<domain>` — no `/api/v1`, no trailing slash |
...
`https://app.<domain>` has to be in the API's `CORS_ORIGIN`. Credentialed CORS refuses
...
Preview builds land on `https://<hash>.swp-contractor.pages.dev`. That origin is **not** in
...
From `https://app.<domain>`:

1. Log in as a `contractor`; DevTools → Application → Cookies shows
   `__Secure-better-auth.session_token` scoped to `.<domain>` (not to `api.<domain>`).
...
5. Upload a photo in step 4 — confirm it round-trips through `POST /api/v1/upload` and renders
   back from `storage.<domain>`. A CORS error here means the bucket CORS step was skipped.
