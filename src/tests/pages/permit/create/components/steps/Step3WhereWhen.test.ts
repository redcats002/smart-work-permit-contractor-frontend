...
/**
 * wayfinder ticket 107/121. Replaces `Step3WhereWhen.pin.test.ts`, which pinned the click-to-place
 * pin behaviour 107 removed entirely — safety places pins now, the contractor only selects one via
 * `PinPicker.vue` (see its own test file). 121 removed the area picker this step used to carry
 * alongside the pin. What remains worth pinning at THIS step's level: it renders cleanly with
 * nothing to pick from, and the 067 UTC trap round-trip.
 */
...
describe('Step3WhereWhen renders cleanly with nothing to pick from (wayfinder 107)', () => {
  ...
  it('zero facility plans, zero pins — no throw, pin/location/dates all still render', async () => {
    setLocale('en')
    stubMatchMedia()
