...
import { formatTitle, isCapitalExpense } from '@/enums/modules/finance/ExpenseType.enum'
...
const items = computed((): IDisplayList[] => {
  const createdBy = props.data?.createdBy.trim() === 'null null' ? '-' : props.data?.createdBy
  const capitalDetail = [
    { label: 'เลขที่', key: 'idNo', value: props.data.idNo },
    { label: 'วันที่', key: 'expenseDate', value: dayjs.formatDate(props.data.expenseDate) },
    { label: 'ผู้ทำรายการ', key: 'createdBy', value: createdBy },
    { label: 'รับ/จ่าย', key: 'type', value: formatTitle(props.data.type) },
    { label: 'สาขาที่อ้างอิง', key: 'referBranchName', value: props.data.referBranchName || '-' },
  ...
  return isCapitalExpense(props.data?.type) ? capitalDetail : paymentDetail
})
</script>
