...
const externalInternalExpenseSchema = z.preprocess((value: unknown): unknown => {
  if (typeof value === 'string') return value
  if (value && typeof value === 'object' && 'id' in value) return (value as { id?: unknown }).id ?? ''
  return value ?? ''
}, z.enum(ExternalInternalExpenseEnum)).refine((val: ExternalInternalExpenseEnum): boolean => val !== '', 'กรุณาเลือกค่าใช้จ่าย ภายใน/ภายนอก')
