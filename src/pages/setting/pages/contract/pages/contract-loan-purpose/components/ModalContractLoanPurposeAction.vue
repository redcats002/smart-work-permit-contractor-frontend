<template>
  <BaseModal
    v-model="visible"
    :label="type==='CREATE' ? 'เพิ่มวัตถุประสงค์การกู้ใหม่' : 'แก้ไขวัตถุประสงค์การกู้'"
    class="md:w-200!"
    @open="clear()">
    <template
      v-if="type === 'CREATE'"
      #activator="{ open }">
      <CreateButton
        label="เพิ่มวัตถุประสงค์การกู้ใหม่"
        @click="open()" />
    </template>
...
import { ContractLoanPurposeSchema, useFormInitialValues } from '../schema/contract-loan-purpose.schema'
...
const resolver = zodResolver(ContractLoanPurposeSchema)
