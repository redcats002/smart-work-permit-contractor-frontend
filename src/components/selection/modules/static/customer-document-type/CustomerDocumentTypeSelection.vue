...
import { CustomerDocumentTypeItems, type TCustomerDocumentType } from '@/enums/modules/customer/CustomerDocumentType.enum'
...
const modelValue = defineModel<TCustomerDocumentType>()
...
const fetchOptions = async (): Promise<TBaseModel[]> => await handleLoading(async (): Promise<TBaseModel[]> => (
  (CustomerDocumentTypeItems ?? []).map((item: TBaseOption): TBaseModel => ({
    id: item.value!,
    name: item?.label
  }))
)) ?? []

const mapOptionToModel = (item: TBaseModel): TCustomerDocumentType | undefined => item?.id ? item.id as TCustomerDocumentType : undefined
</script>
