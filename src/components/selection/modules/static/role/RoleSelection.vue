...
const modelValue = defineModel<TEmployeeRole>()
...
const fetchOptions = async (): Promise<TBaseModel[]> => await handleLoading(async (): Promise<TBaseModel[]> => (
  (EmployeeRoleItems ?? []).map((item: TBaseOption): TBaseModel => ({
    id: item.value!,
    name: item?.label
  }))
)) ?? []

const mapOptionToModel = (item: TBaseModel): TEmployeeRole | undefined => item?.id ? item.id as TEmployeeRole : undefined
</script>
