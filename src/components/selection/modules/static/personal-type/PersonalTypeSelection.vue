...
import { PersonalTypeItems, type TPersonalType } from '@/enums/modules/customer/PersonalType.enum'
...
const modelValue = defineModel<TPersonalType>()
...
const fetchOptions = async (): Promise<TBaseModel[]> => await handleLoading(async (): Promise<TBaseModel[]> => (
  (PersonalTypeItems ?? []).map((item: TBaseOption): TBaseModel => ({
    id: item.value!,
    name: item?.label
  }))
)) ?? []

const mapOptionToModel = (item: TBaseModel): TPersonalType | undefined => item?.id ? item.id as TPersonalType : undefined
</script>
