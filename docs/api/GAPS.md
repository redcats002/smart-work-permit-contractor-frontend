...
| J | ~~**The permit has no field for step 3's Yes/No/N-A safety checklist.**~~ **Closed 2026-09-12.** `PATCH /permits/:id` now accepts an optional `preWorkChecklist: Array<{ itemKey, answer }> \| null` — omitting the key leaves the stored value untouched (same presence-vs-value convention as `jsaSteps`/`photos`), `null` clears it. The read side (`GET /permits`, `GET /permits/:id`) echoes it back in the same shape, same convention as `closureChecklist`. | **Frontend half closed in the same change.** `useWizard.doPersist` maps `checklistAnswers` to the wire array on every autosave (`SafetyChecklist.ts`'s `toPreWorkChecklistPayload`); `hydrate` rehydrates it from `permit.preWorkChecklist` (`toChecklistAnswerMap`) so a resumed draft keeps its answers instead of landing on 17/13/14 blank rows. The on-screen disclaimer (`permit.create.steps.safetyChecks.checklistNotStored`, which was true and is now false) is replaced by `checklistSavedWithDraft`. No gating added — product ruling: this stays a pre-work reference aid, never a client- or server-side block on submit. |
